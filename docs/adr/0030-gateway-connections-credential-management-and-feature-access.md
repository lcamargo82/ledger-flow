...
2. **Criptografia Simétrica (AES-256-GCM)**:
   Decidimos utilizar AES-256-GCM com chave-mestra configurada no ambiente (variável `ENCRYPTION_KEY`) para criptografar as credenciais (`apiKey`, `secret`, etc) antes de armazenar no banco. O payload é mantido no formato encriptado e só decodificado em memória no momento da comunicação com os provedores via adapters. O hash (fingerprint) é mantido para detecção de duplicidade ou validação.

3. **Controle de Acessos com TenantFeatureAccessService**:
...
4. **Escopos de Permissão Isolados**:
...
- Segurança superior impedindo vazamento de secrets de gateway (em logs, repositórios ou banco exposto).

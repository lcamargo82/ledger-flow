...
export abstract class PlatformTenantsRepository {
  ...
  abstract findGatewaySummaryByTenant(tenantId: string): Promise<{ hasActiveConfiguration: boolean; activeProviders: any[] }>;
  abstract findWebhookSummaryByTenant(tenantId: string, sinceDate: Date): Promise<{ lastReceivedAt: Date | null; processed: number; failed: number; ignored: number }>;

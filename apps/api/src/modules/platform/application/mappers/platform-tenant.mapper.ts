...
export class PlatformTenantMapper {
  static toResponseDto(tenant: Tenant & { subscription?: TenantSubscription | null }): PlatformTenantResponseDto {
    return {
      id: tenant.id,
      name: tenant.name,
      slug: tenant.slug,
  ...
  static toDetailsResponseDto(
    tenant: Tenant & { subscription?: TenantSubscription | null; _count: { users: number; customers: number; payments: number } }
    ...
    if (tenant.subscription) {
      response.subscriptionDetails = {
        ...
        externalSubscriptionReference: tenant.subscription.externalSubscriptionReference || undefined,

...
export class AsyncJobRetryService {
  ...
  async scheduleRetry(executionId: string, attemptCount: number, eventType: string, payload: any): Promise<void> {
    const delayMs = ASYNC_RETRY_POLICY.DELAYS_MS[attemptCount - 1] || ASYNC_RETRY_POLICY.DELAYS_MS[ASYNC_RETRY_POLICY.DELAYS_MS.length - 1];

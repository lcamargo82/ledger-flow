...
export class AuthService {
  ...
  async login(loginDto: LoginDto, ipAddress?: string, userAgent?: string): Promise<AuthResponse> {
    ...
    try {
      ...
      const capabilities = await this.capabilityPolicy.getCapabilitiesForTenant(user.tenantId);
      ...
      const payload: AuthTokenPayload = {
        ...
        email: user.email,
        isPlatformAdmin: user.isPlatformAdmin || roles.includes('PLATFORM_OWNER'),
        roles,
        permissions,
        capabilities,
        sessionId: sessionId!,
  ...
  async refresh(refreshTokenDto: RefreshTokenDto): Promise<{ accessToken: string }> {
    ...
    const payload: AuthTokenPayload = {
      ...
      isPlatformAdmin: storedToken.user.isPlatformAdmin || roles.includes('PLATFORM_OWNER'),
      roles,
